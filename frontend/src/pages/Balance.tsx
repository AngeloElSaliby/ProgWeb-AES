...
const Balance: React.FC = () => {
  ...
  const globalBalance = data.find((item: UserBalance) => item.with === "all")
    ?.balance || { owedToUser: "0", owedByUser: "0" };

  const userBalances = data.filter((item: UserBalance) => item.with !== "all");

  const hasOwedToOthersData =
    parseFloat(globalBalance.owedByUser) > 0 ||
    parseFloat(globalBalance.owedToUser) > 0;
  const hasOwedToOthersByUserData = userBalances.some(
    (item: UserBalance) =>
      parseFloat(item.balance.owedByUser) -
        parseFloat(item.balance.owedToUser) >
      0
  );
  const hasOwedByOthersByUserData = userBalances.some(
    (item: UserBalance) =>
      parseFloat(item.balance.owedToUser) -
        parseFloat(item.balance.owedByUser) >
      0
    ...
    datasets: [
      {
        label: "Total Balance",
        data: [
          parseFloat(globalBalance.owedToUser),
          parseFloat(globalBalance.owedByUser),
        ],
  ...
  const chartDataOwedByOthersByUser = {
    labels: userBalances
      ...
      .map((item: UserBalance) => item.with),
    datasets: [
      {
        label: "Owed by",
        data: userBalances
        ...
        backgroundColor: userBalances
          .filter(
            (item: UserBalance) =>
              parseFloat(item.balance.owedToUser) -
              parseFloat(item.balance.owedByUser)
          )
          .map((item: UserBalance) => getColor(item.with)), //rescaling to max rgb
      },
    ],
  };
    ...
    datasets: [
      {
        ...
        backgroundColor: userBalances
          ...
          .map((item: UserBalance) => getColor(item.with)),
      },
    ],
  };
      ...
      <div className="flex flex-col space-y-8">
        <div className={CardStyle}>
          ...
          <h3 className="text-l font-semibold">
            You owe: €{globalBalance.owedByUser}, you are owed €
            {globalBalance.owedToUser}
          </h3>
          {hasOwedToOthersData ? (
